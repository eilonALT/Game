...
public class MainTrain {
	...
	public static void client1(int port) throws Exception {
		...
		if (response == null || !response.equals(rev))
			...
			System.out.println("got the right response from your server, continue the test");
		}
		in.close();
		outToServer.println(text);
		outToServer.close();
	...
	public static boolean testServer() {
		...
		try {
			client1(port);
			System.out.println("client1 passed");
		} catch (Exception e) {
		...
		if (Thread.activeCount() != c) {
			...
			System.out.println("you closed all threads after calling close method");
		}
		return ok;
	}
	...
	public static void testDM() {
		...
		if (!dm.challenge("t1.txt", "t2.txt", "t3.txt", t3[2]))
			System.out.println("problem for Dictionary Manager challenge (-5) 5");
		else
			System.out.println("challenge 1 passed");
		if (dm.challenge("t2.txt", "t3.txt", "t1.txt", "3" + t2[5]))
			System.out.println("problem for Dictionary Manager challenge (-5) 6");
		else
			System.out.println("challenge 2 passed");
		if (dm.getSize() != 3)
